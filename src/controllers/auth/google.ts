...
export const googleAuthCallback = (req: Request, res: Response, next: NextFunction) => {
	passport.authenticate(
		'google',
		{ session: false },
		async (err: { message?: string }, user: Express.User | false, info?: { message?: string }) => {
			try {
				if (err) {
					return res.redirect(getErrorRedirect('auth', { errorMessage: err.message }));
				}

				if (!user) {
					const errorKind = info?.message?.toLowerCase().includes('state') ? 'state_mismatch' : 'auth';
					return res.redirect(getErrorRedirect(errorKind, { errorMessage: info?.message }));
				}
