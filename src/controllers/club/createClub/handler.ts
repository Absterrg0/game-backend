...
export async function createClubFlow(data: CreateClubInput, userId: string) {
  ...
  try {
    const club = await createClubWithSession(
      {
        ...
        tennisLessonRequestEmail: data.tennisLessonRequestEmail?.trim() || null,
        membershipRequestEmail: data.membershipRequestEmail?.trim() || null,
    ...
    const adminAssignmentResult = await addUserAdminOfClub(
      club._id.toString(),
      userId,
      session,
    );
    ...
    if (
      ...
      await session.abortTransaction();
      session.endSession();
      return error(500, "Failed to assign club admin role to club creator");
    }
