...
function getSetFormat(playMode: GamePlayMode, setIndex: number): "standard" | "tiebreak10" {
...
function compareSetScore(
  ...
  if (setFormat === "standard") {
    // Validate bounds for standard set
    if (playerOneScore > 7 || playerTwoScore > 7) {
      throw new AppError(`Invalid standard set score: ${playerOneScore}-${playerTwoScore}. Maximum games allowed is 7.`, 400);
    }
    if ((playerOneScore === 7 && playerTwoScore < 5) || (playerTwoScore === 7 && playerOneScore < 5)) {
      throw new AppError(`Invalid standard set score: ${playerOneScore}-${playerTwoScore}. A 7-game win must have an opponent at 5 or 6 games.`, 400);
    }
    
    // Evaluate if there is a winner
    if ((playerOneScore === 6 && playerTwoScore <= 4) || (playerOneScore === 7 && (playerTwoScore === 5 || playerTwoScore === 6))) {
      return 1;
    }
    if ((playerTwoScore === 6 && playerOneScore <= 4) || (playerTwoScore === 7 && (playerOneScore === 5 || playerOneScore === 6))) {
      return -1;
    }
  ...
  if (setFormat === "tiebreak10") {
    if (playerOneScore >= 10 && playerOneScore - playerTwoScore >= 2) {
      if (playerOneScore > 10 && playerOneScore - playerTwoScore > 2) {
        throw new AppError(`Invalid TieBreak10 score: ${playerOneScore}-${playerTwoScore}. Point difference should not exceed 2 if going past 10 points.`, 400);
      }
      return 1;
    }
...
function decisiveSetsLength(playMode: GamePlayMode, input: RecordMatchScoreInput): number | null {
  ...
  for (let index = 0; index < setsToEvaluate; index += 1) {
    ...
    const setResult = compareSetScore(playerOneScore, playerTwoScore, playMode, index);
    if (setResult > 0) {
      playerOneSetWins += 1;
    } else if (setResult < 0) {
      playerTwoSetWins += 1;
