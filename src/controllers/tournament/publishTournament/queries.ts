...
export async function fetchTournamentForPublish(id: string) {
  try{
    ...
    .select("_id club createdBy status name sponsor logo date startTime endTime playMode tournamentMode entryFee minMember maxMember duration breakDuration courts foodInfo descriptionInfo")
    .lean()
    .exec();
    if(!tournament){
